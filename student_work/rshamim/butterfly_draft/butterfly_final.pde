...
void draw (){
 ...
 fill(0,10);
  ...
  if((width * posPercentage /25) < width){
  backgroundimg.blend(backgroundimg2,0, 0,
                      int((width * posPercentage / 25)), height,
                     0, 0,
                      int((width * posPercentage /25)), height, BLEND);
  } else /* if ((width * posPercentage /25)==width)*/{
     
     backgroundimg2.blend(backgroundimg3,0, 0,
                    (int) ((width * posPercentage /60)-width), height,
                    0, 0,
                     (int) ((width * posPercentage / 60)-width), height, BLEND); 
   }
