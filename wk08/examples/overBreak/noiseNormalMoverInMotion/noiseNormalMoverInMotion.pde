...
// 
// z-axis of the noise() set with variabls z and zStep


// adjusts the noise scale
float scale=.125;

// adjusts noise animation
float z = 0;
float zStep = .1;
...
void generateForceField() {
  noise = drawNoise(width/div, height/div, z);
...
PImage drawNoise(int _w, int _h, float _z) {
  ...
  for ( int y = 0; y < image.height; y++ ) {
    for ( int x = 0; x < image.width; x++ ) {
      ...
      image.pixels[ y * image.width + x ] = color( map( noise( x * scale, y * scale, scale * _z ), 0, 1, 0, 255 ) );
    }
  }
  image.updatePixels();
  return image;
...
PVector calculateNormal(PImage _image, PVector _position) {
  ...
  for (int i = 0; i < points.length; i++) {
    float x = r*cos(i*2*PI/3)+_position.x;
    float y = r*sin(i*2*PI/3)+_position.y;
